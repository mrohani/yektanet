...
if (file_exists($jsonFilePath)) {
    ...
    foreach ($items as $index => $item) {
        ...
        if ($imageData) {
            ...
            // Generate new image name: digi-img-{id}-{random_5_digits}.png
            $id = $index + 1;
            $randomNumber = rand(10000, 99999);
            $newImageName = "digi-img-{$id}-{$randomNumber}.png";
            $newImagePath = 'images/' . $newImageName;

            // Save resized image to file as PNG (compression level: 6 out of 9)
            imagepng($resizedImage, $newImagePath, 6);
